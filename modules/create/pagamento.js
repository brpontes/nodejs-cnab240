...
module.exports = (( shipping ) => {
    ...
    const header_file = () => {
        ...
        header += `${HEADER.HORA_GERACAO}${HEADER.NSA}${HEADER.VERSAO_LEIAUTE_PAG_HEADER}${leftPad(' ', 74)}${os.EOL}`
    ...
    const segmento_a = ( shipping_detail ) => {
        ...
        segmento += `${shipping_detail.cod_instrucao}${shipping_detail.cod_camara}${shipping_detail.banco_favorecido}${leftPad(shipping_detail.agencia_favorecido, 6, '0')}`
        segmento += `${leftPad(shipping_detail.conta_favorecido, 13, '0')} ${rightPad(shipping_detail.nome, 30, ' ')}${rightPad(shipping_detail.seu_numero, 20, ' ')}`
        segmento += `${shipping_detail.data_pgto}${DETAIL.TIPO_MOEDA}${leftPad(DETAIL.QTDE_TIPO_MOEDA, 15, '0')}${shipping_detail.valor_pgto}`
        segmento += `${rightPad(shipping_detail.nosso_numero, 20, ' ')}${leftPad('0', 23, '0')}${rightPad(shipping_detail.informacao_judicial, 40, ' ')}`
        segmento += `${rightPad(shipping_detail.compl_tipo_servico, 2, ' ')}${rightPad(shipping_detail.finalidade_ted, 5, ' ')}`
        segmento += `${rightPad(shipping_detail.compl_finalidade_pgto, 2, ' ')}${rightPad(' ', 3, ' ')}${shipping_detail.cod_aviso}${rightPad(' ', 10, ' ')}${os.EOL}`
    ...
    const segmento_b = ( shipping_detail ) => {
        ...
        segmento += `${rightPad(shipping_detail.logradouro, 30, ' ')}${leftPad(shipping_detail.numero, 5, '0')}${rightPad(' ', 15, ' ')}${rightPad(shipping_detail.bairro, 15, ' ')}`
        segmento += `${rightPad(shipping_detail.cidade, 20)}${shipping_detail.cep}${shipping_detail.uf}${shipping_detail.data_pgto}${shipping_detail.valor_pgto}`
        segmento += `${shipping_detail.abatimento}${shipping_detail.desconto}${shipping_detail.juros}${shipping_detail.multa}${rightPad(' ', 30, ' ')}${os.EOL}`

        return segmento
    }

    const trailer_lote = ( shipping ) => {
        //Quantidade de registros * 2 (QUANTIDADE DE SEGMENTOS) + 2 (HEADER DE ARQUIVO/LOTE)
        const SHIPPING_STRING_LENGTH = shipping.length * 2 + 2
        const SOMA_VALORES = shipping.reduce((prev, curr) => prev + curr.valor_pgto * 1, 0)

        let trailer = `${HEADER.COD_BANCO}${HEADER.LOTE_SERVICO_LOTE}5${leftPad('', 9)}`
        trailer += `${leftPad(SHIPPING_STRING_LENGTH, 6, '0')}${leftPad(SOMA_VALORES, 18, '0')}${leftPad('', 199)}${os.EOL}`
    ...
    const trailer_file = ( shipping ) => {
        ...
        const SHIPPING_STRING_LENGTH = shipping.length * 2 + 4
